# 🚀 GUIDE COMPLET - EXÉCUTION SUR MACHINE LOCALE

## 📋 CONTEXTE

Vous avez une application en production sur Vercel avec:
- ✅ Backend fonctionnel avec base de données Supabase
- ✅ Frontend Angular déployé
- ✅ Infrastructure cache complète créée (PHASE 1-4)
- ⏸️ Migration Prisma `add_updated_at_fields` non appliquée
- ⏸️ 4 services à adapter (entrainement, tag, echauffement, situationmatch)
- ⏸️ PreloadService à créer
- ⏸️ Tests à effectuer
...
Appliquer toutes les modifications du système de cache multi-niveaux sur votre machine locale, puis déployer en production.
...
## 📦 ÉTAPE 1: RÉCUPÉRER LE CODE (5 min)
...
Fichiers créés lors de notre session:

**Infrastructure Cache:**
- `frontend/src/app/core/models/cache.model.ts`
- `frontend/src/app/core/services/indexed-db.service.ts`
- `frontend/src/app/core/services/data-cache.service.ts` (modifié)
- `frontend/src/app/core/services/sync.service.ts`

**Backend:**
- `backend/routes/sync.routes.js`
- `backend/routes/index.js` (modifié)
- `backend/prisma/schema.prisma` (modifié - champ `updatedAt` ajouté)
- `backend/prisma/migrations/add_updated_at_fields.sql`

**Services Adaptés:**
- `frontend/src/app/core/services/auth.service.ts` (modifié)
- `frontend/src/app/core/services/workspace.service.ts` (modifié)
- `frontend/src/app/core/services/exercice.service.ts` (modifié)
...
## 🗄️ ÉTAPE 2: CONFIGURATION BACKEND (10 min)
...
### 2.2 Éditer backend/.env avec vos vraies valeurs

Ouvrir `backend/.env` et remplacer par:

```env
# Base de données Supabase (Transaction mode pour production)
...
## 🔄 ÉTAPE 3: MIGRATION PRISMA (5 min)

### 3.1 Vérifier la Version Prisma
...
**Option A: Migration Dev (Recommandé pour local)**

```powershell
npx prisma migrate dev --name add_updated_at_fields
```

**Option B: Si la migration existe déjà en production**

```powershell
npx prisma migrate deploy
```

**Option C: Si problème, utiliser db push**
...
### 3.4 Vérifier avec Prisma Studio (Optionnel)

```powershell
npx prisma studio
```

Ouvrir http://localhost:5555 et vérifier que le champ `updatedAt` existe sur:
- Exercice
- Tag
- Entrainement
- Echauffement
...
## 🚀 ÉTAPE 4: DÉMARRER LE BACKEND (2 min)
...
**Si le serveur démarre correctement, la migration est RÉUSSIE !** ✅

---

## 🎨 ÉTAPE 5: CONFIGURATION FRONTEND (5 min)

### 5.1 Installer les Dépendances Frontend
...
## 📝 ÉTAPE 6: ADAPTER LES 4 SERVICES RESTANTS (1h)
...
// Même logique pour update, delete
```
...
Même pattern que EntrainementService.
...
## 🔄 ÉTAPE 7: CRÉER PRELOADSERVICE (30 min)
...
### 7.2 Intégrer dans AppComponent
...
# Test versions (nécessite token)
# Récupérer un token via login, puis:
...
4. Vérifier que la base `ufm-cache` existe avec les stores:
   - auth
   - workspaces
   - exercices
   - entrainements
...
3. Dans l'onglet 2: Vérifier que la liste se met à jour automatiquement
4. Console onglet 2:
   ```
...
## 📊 ÉTAPE 9: VÉRIFICATION FINALE (15 min)

### 9.1 Checklist Backend

- [ ] Migration Prisma appliquée avec succès
- [ ] Champ `updatedAt` présent sur toutes les tables
- [ ] Endpoint `/api/sync/health` retourne OK
- [ ] Endpoint `/api/sync/versions` retourne les timestamps
- [ ] Serveur démarre sans erreur

### 9.2 Checklist Frontend

- [ ] IndexedDB initialisé avec tous les stores
- [ ] Cache profil fonctionne (F5 charge depuis cache)
- [ ] Cache exercices fonctionne (HIT après premier chargement)
...
- [ ] ExerciceService adapté ✅
- [ ] EntrainementService adapté
- [ ] TagService adapté
- [ ] EchauffementService adapté
- [ ] SituationMatchService adapté
- [ ] PreloadService créé et intégré

---

## 🚀 ÉTAPE 10: DÉPLOIEMENT PRODUCTION (15 min)
...
### 10.4 Vérifier la Migration en Production
...
**Solution**: Normal si déjà fait en production. Utiliser `npx prisma migrate deploy`
...
**Solution**: Vérifier dans DevTools → Application → IndexedDB que `ufm-cache` existe
...
✅ **Système de cache multi-niveaux complet**
...
**Bonne chance pour l'exécution sur votre machine locale !** 🚀

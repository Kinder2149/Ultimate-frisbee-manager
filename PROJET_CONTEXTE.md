...
Un agent en tâche de fond a été lancé pour : (A) importer les ~49 Thèmes/Phases/Sous-phases de Notion comme `Tag` (catégorie `theme_entrainement` pour les thèmes, `phase_entrainement` avec `parentId` pour les phases/sous-phases — "Le jeu long" importée avec `parentId: null`, décision Kinder toujours en attente) ; (B) importer les 73 Exercices vers le modèle `Exercice`, avec parsing du corps de chaque fiche Notion (sections Objectif/Déroulement → `description`, Critère de réussite, Variable +/-, Matériel, Durée, Effectif) et tags (`travail_specifique` depuis la propriété du même nom, `objectif` depuis Type d'exercice + Éléments Travaillés, `phase_entrainement` depuis la relation Phases). Script one-off : `backend/scripts/import-ulti-coach-exercices.js` (peut être supprimé après coup, ce n'est pas un fichier de doc). Images Notion **non importées** (URLs signées, expirent en 5 min — nécessiterait un re-upload Cloudinary, hors scope).

**⚠️ Quel workspace ?** Tout est créé dans **"Ulti Coach"**, `workspaceId = "e133fed1-ab18-4e1f-8e10-eb9124645fa6"` — jamais dans "BASE" (workspace protégé, réservé aux admins plateforme, écriture refusée de toute façon par `baseMutationGuard`). C'était explicite dans les instructions données à l'agent (`Tag.workspaceId` et `Exercice.workspaceId` fixés sur cet id). Pour vérifier à l'écran : se connecter, "Changer d'espace" → choisir **Ulti Coach** (rôle Gestionnaire pour Kinder) avant de regarder `/tags` ou `/exercices` — si on reste sur "BASE" par défaut, tout paraîtra vide alors que les données sont bien là.

**Statut exact au moment de la pause : à vérifier en premier à la reprise.** Le rapport de l'agent n'était pas encore revenu quand la session a été mise en pause. **Important : cet agent tourne dans cette session Claude Code — s'il se termine après la fin de la session (fermeture, changement de machine), son rapport peut ne jamais arriver.** Ne pas se fier uniquement à une notification qui n'apparaît pas : vérifier directement l'état réel, indépendamment de la session, avec l'une de ces méthodes :
- **Dans l'appli** : se connecter, passer sur le workspace "Ulti Coach", ouvrir `/tags` (onglets Thèmes Entraînements / Phases — doivent afficher ~10 et ~39 tags si l'étape A est terminée) et `/exercices` (doit afficher jusqu'à 73 si l'étape B est terminée).
- **En base, sans repasser par l'agent** (nouvelle session Claude Code, ou n'importe quel outil SQL connecté à Supabase) :
  ```sql
  SELECT category, COUNT(*) FROM "Tag" WHERE "workspaceId" = 'e133fed1-ab18-4e1f-8e10-eb9124645fa6' GROUP BY category;
  SELECT COUNT(*) FROM "Exercice" WHERE "workspaceId" = 'e133fed1-ab18-4e1f-8e10-eb9124645fa6';
  ```
  Si le compte d'exercices est entre 1 et 72, l'import s'est arrêté en cours de route (crash, ou session terminée trop tôt) — relancer une mission équivalente en réutilisant le mapping documenté ci-dessus plutôt que de repartir de zéro. Si `backend/scripts/import-ulti-coach-exercices.js` existe encore, le relire donne l'état exact du script au moment où il s'est arrêté.

...
6. ~~**Alignement Notion ↔ UFM (audit du 2026-09-11)**~~ ✅ **fait le 2026-09-11** — les deux bases sont identiques :
   - Catalogue : `backend/scripts/aligner-ulti-coach-catalogue.js` (référence `scripts/data/ulti-coach-notion-catalogue.json`, 77 fiches Notion) — 66 fiches issues des séances supprimées (+ doublon « Fléche »), échauffements purs et étirements rangés dans la famille Échauffements, tags recalculés depuis Notion. Résultat : **41 exercices + 36 échauffements = 77 fiches Notion**, 41 phases.
   - Images : `backend/scripts/images-ulti-coach.js <lot.json>` (URL signée Notion → Cloudinary, dossier `ultimate-frisbee-manager/{exercices|echauffements}`). 42 fiches illustrées (17 exercices, 25 échauffements). Limite UFM : une seule image par fiche (Réveil musculaire en a 7 dans Notion, Zone Carré 3, Braise/Diago/Praline 3).
   - Séances : `backend/scripts/import-ulti-coach-seances-complement.js` (données `scripts/data/ulti-coach-seances-complement.json`) — les 20 séances restantes, repérées par la nouvelle catégorie de tag **« Statut de la séance »** (`statut_seance`) : 4 « À valider » + 16 « Archive 2023-2024 ». **61 séances = 61 pages Notion.** Une séance n'est reliée à une fiche que si la relation Notion « Exercices » le fait (13 séances, liens identiques à Notion) ; le reste du texte (plans de coaching, bilans, exercices cités) est dans la situation liée (« Match — … » ou « Contenu — … »). Le repère s'affiche sur l'écran Déroulement de l'année.
   - **Plusieurs images par fiche** (2026-09-12) : l'image principale reste `imageUrl`, les suivantes vont dans `imagesSupplementaires` (migration `20260911180000_add_images_supplementaires`, exercices / échauffements / situations). Le formulaire d'une fiche a une section « Images supplémentaires » (ajout, ordre, retrait) ; l'envoi passe par `POST /api/{exercises|warmups|matches}/images` qui renvoie l'adresse Cloudinary. Les fiches affichent toutes leurs images, la vignette des listes garde la première. Galeries reprises de Notion : Réveil musculaire (7), Zone Carré (3), Braise (3), Diago (3), Praline (3). Tests : `backend/__tests__/galerie-images.test.js`.
   - **Familles d'échauffement** (2026-09-12) : le modèle `Echauffement` a désormais des étiquettes (`_EchauffementTags`, migration `20260912090000_add_echauffement_tags`) et une nouvelle catégorie `type_echauffement` (« Physique », « Réveil musculaire »). Les 36 échauffements du catalogue sont classés par le script `backend/scripts/classer-echauffements.js` (règle : fiche Notion « Étirement » → Réveil musculaire, « Échauffement » → Physique ; 3 cas particuliers écrits dans le script) → 13 physiques, 23 réveil musculaire, 0 non classé. La page Échauffements filtre par famille, la fiche et le formulaire affichent l'étiquette. Tests : `backend/__tests__/echauffement-tags.test.js`.
   - Pas de synchronisation automatique : toute modification future doit être faite des deux côtés (ou décider d'une source unique).
